import { deleteMemberItem } from '@api/member'
...
const Index: FC<{
  ...
  const isVisit = detail?.service_id === 1
  const [btnLoading, setBtnLoading] = useState<boolean>(false)

  const handleDelete = () => {
    if (detail?.id) {
      setBtnLoading(true)
      deleteMemberItem(detail?.id)
        .then(({ data }: any) => {
        ...
        <div className=''>
          <div className='p-15px d-flex flex-center h-100px'>
            <div className='text-center fs-14px'>
              ...
              <span className='fw-bolder mx-5px'>{isVisit ? 'Gym Visit' : detail?.name}</span>
              <span className=''>?</span>
            </div>
          </div>
